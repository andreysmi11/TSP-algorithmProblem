...
class TSPApprox extends TSPAlg {
	...
	public void doNextStep() {
		...
		if(step == STEP_MST){
			...
			DelegateForest<City,Flight> Dforrest = new DelegateForest<>();
			HashMap<Flight,Double> Flightmap = new HashMap<Flight,Double>();


			for(City c1: graph.getVertices()) {
				for(City c2: graph.getVertices()) {
					if(!c1.equals(c2)) {
						if(graph.isNeighbor(c1, c2)) {
							Flight f = graph.findEdge(c1, c2);
							Double fcost = Double.valueOf(f.getCost());
							Flightmap.put(f, fcost);
						}
					}
				}
			}
			...
			MinimumSpanningForest<City,Flight> minTree = new MinimumSpanningForest<City,Flight>(super.graph, Dforrest, startingCity, Flightmap);
			tree = minTree.getForest();
			colorAllEdges(tree, COLOR_ACTIVE_EDGE);
		}
		if(step == STEP_DFT){
			super.visitOrder = new ArrayList<City>();
			super.visitOrderCost = 0;
			depthFirstTraversal(tree, startingCity, visitOrder);
			City sCity = startingCity;
			for(City c: visitOrder){
				if(!c.equals(sCity))
					super.visitOrderCost = super.visitOrderCost + super.graph.findEdge(c, sCity).getCost();
				sCity = c;
			}

			super.visitOrderCost = super.visitOrderCost + super.graph.findEdge(sCity, startingCity).getCost();
		}
		/*
		Basic instructions:
	...
	static void depthFirstTraversal(Forest<City,Flight> treeToWalk, City currNode, List<City> dftVisitOrder) {
		...
		if(currNode.equals(null)){
			...
			Collection<Flight> flist = treeToWalk.getChildEdges(currNode);
			for(Flight f : flist) {
				City child = treeToWalk.getOpposite(currNode, f);
